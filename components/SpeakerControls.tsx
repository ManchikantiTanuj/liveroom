...
export default function SpeakerControls({
  ...
  const startRecording = useCallback(async () => {
    try {
      ...
      ws.onerror = (e) => {
        ...
        alert('Deepgram connection failed. Check your API key.')
        stream.getTracks().forEach(t => t.stop())
  ...
  async function testMic() {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true })
      stream.getTracks().forEach(t => t.stop())
      alert('Microphone works! If recording still fails, check NEXT_PUBLIC_DEEPGRAM_API_KEY in Vercel.')
    } catch (err: any) {
      ...
      {!isRecording && (
        <div className="flex items-center gap-3 mt-2">
          ...
          <span className="text-xs font-mono text-zinc-800">
            Deepgram: {process.env.NEXT_PUBLIC_DEEPGRAM_API_KEY ? 'set' : 'missing'}
          </span>
        </div>
      )}
    </div>
